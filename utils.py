...
from data import ImageFilelist, ImageFolder_with_attributes
import torch
...
def get_data_loader_folder(input_folder, batch_size, train, new_size=None,
    ...
    if config['use_genders']:
        _, domain = os.path.split(input_folder)
        if domain == 'trainA':
            attr_path = config['celeba_genders_train_path']
        elif domain == 'trainB':
            attr_path = config['bitmoji_genders_train_path']
        elif domain == 'testA':
            attr_path = config['celeba_genders_test_path']
        elif domain == 'testB':
            attr_path = config['bitmoji_genders_test_path']
        dataset = ImageFolder_with_attributes(input_folder, transform=transform, attr_path=attr_path)
